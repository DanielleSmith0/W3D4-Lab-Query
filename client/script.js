...
function btnColor (){
    sayHelloButton.classList.add("say-hello-button");
}
...
function btnColorOriginal (){
    sayHelloButton.classList.remove("say-hello-button");
}

sayHelloButton.addEventListener("mouseout", btnColorOriginal);
...
const repeatMyParam = () => {
    axios.get(`${baseURL}/repeat/string`).then(result => {
        console.log(result.data);
        const repeatText = document.querySelector("#repeat-text");
        repeatText.textContent = result.data;
    }).catch(err => console.log({err, message: "error with repeatMyParam"})); 
}
